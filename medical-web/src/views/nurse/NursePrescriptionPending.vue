  ...
  <div class="page">
    <div class="page-header">
      <div class="header-left">
        ...
        <div>
          <h2 class="page-title">待发药</h2>
          <p class="page-desc">查看待发药处方，确认后完成发药并扣减库存。</p>
        </div>
      </div>
    </div>
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await getPendingPrescriptions(keyword.value || undefined)
...
const dispense = async (row) => {
  ...
  if (detailVisible.value && detail.value?.prescriptionId === row.prescriptionId) {
